...
class webdata_Model extends CI_Model {
  ...
  function domainCommentsPagination(){
    ...
    $query_str = mysql_query("SELECT count(page) as n_pages from page where domain='$currentDomain'");
    $query_str = mysql_fetch_array($query_str);
    $itemAmount = $query_str["n_pages"];
    $config['total_rows'] = $itemAmount;
    $config['per_page'] = 10;
    $config['uri_segment'] = 3;  // 表示第 3 段 URL 为当前页数，如 index.php/控制器/方法/页数，如果表示当前页的 URL 段不是第 3 段，请修改成需要的数值。
    $config['prefix'] = 'index/';
  ...
  function getArticleByDomain($domainURL,$from_item){
    $query_str = "SELECT * FROM page
      WHERE page.domain = '$domainURL' ORDER BY page.time DESC LIMIT $from_item,10";
...
echo $subject*/
  ...
  function userAddToBlack(){
    ...
    if($user_id !=''&&$user_id!=100){
      ...
      if(mysql_num_rows($state)==0){	
        if(trim($user_email)==''||$user_email.length<=0){$emailStr ='';$emailStrA ='';}else{$emailStr = ',email';$emailStrA = ",'".$user_email.",'";}
          $user_name = $user_name.',';
