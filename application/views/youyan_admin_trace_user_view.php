...
<div class="contentWrapper">
    ...
    <div class="contentContainer" id="basicContainer">    
         <div class="leftWrapper">
            ...
            <a class="adminLeftMenuBTN currentLeftMenu" >用户分析</a>
...
<!--start-->   
<?php 
	$strforlinkb = strtolower($row->from_type)."_id";
?>   
<script language="javascript" type="text/javascript">	
	$prefixLinkA = SNSTypeToPrefix['<?php echo $row->from_type;?>'];	
	$prefixLinkB = '<?php echo getTargetID($row,$strforlinkb);?>';	

	$fromTypeLink = $prefixLinkA+$prefixLinkB;  
...
?>          </div>
       ...
       <div class="rightWrapper">
            ...
            <div class="checkTable">
            	<div class="firstLineTable"><div class="checkNum"></div><div class="checkTitleUp">用户名</div><div class="checkComment">评论量</div><div class="checkTrace">带来回访量</div><div class="clear"></div></div>
                <div class="checkItem"><div class="checkNumAll">0</div><a class="checkTitleAll" href="javascript:prepareData(7,'<?php echo $_SESSION['showDomain'];?>');backTitle('user');">全部用户</a><div class="checkComment"><?php echo $domainTrace[0]->n_comments;?></div><div class="checkTrace"><?php if($domainTrace[0]->traceamount==NULL){echo 0;}else{echo $domainTrace[0]->traceamount;}?></div><div class="clear"></div></div> 
                <?php 
					$numItem = 1;
				foreach($userTrace as $row){
                if($row->show_name!=''){?>  
             	<?php 
					$from_type = '';
					for($i=0;$i<=7;$i++){
						if(isset($sns_array)&&$sns_array!=''&&$sns_array!==NULL){
							 if($row->$sns_array[$i]!=''&&$row->$sns_array[$i]!='0'){
								$from_type = $change_array[$i];	
								break;
							 }
						}
				...
				?>              
    <script language="javascript"> 
	//prepare url for user link;
	//from_type example SINA
	//fromLinkB exapmle 1835069095
		var $fromLinkA = SNSTypeToPrefix['<?php echo $from_type;?>'];
		<?php $userSendType = strtolower($from_type)."_id";?>
		var $fromLinkB = '<?php  echo getTargetID($row,$userSendType);?>';
		fromTypeLink[<?php echo $numItem;?>] = $fromLinkA+$fromLinkB;

    </script>
    <div class="checkItem"><div class="checkNum"><?php echo $numItem;?></div><a class="checkTitle" href="javascript:<?php if($from_type!=''){?>changeLink(fromTypeLink[<?php echo $numItem;?>]);<?php }else{?>clearLink();<?php }?>changeData(7,'<?php echo $row->user_id;?>','user');changeTitle('<?php echo $row->show_name;?>','<?php echo $row->user_id;?>','user');"><?php if($from_type==''){echo '<span class="emailUserName">'.$row->show_name;}else{echo $row->show_name;}?></a><div class="checkComment"><?php echo $row->usercommentamount;?></div><div class="checkTrace"><?php if($row->traceamount==NULL){echo 0;}else{echo $row->traceamount;}?></div><div class="clear"></div></div><?php $numItem++;?>     
