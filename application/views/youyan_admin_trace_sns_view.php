...
<div class="contentWrapper">
    ...
    <div class="contentContainer" id="basicContainer">
        <div class="leftWrapper">
            ...
            <a class="adminLeftMenuBTN currentLeftMenu">各社交网站回访</a>
            </div>
...
?>          </div>
            ...
            <!--<div class="kindHearted leftcube">
                <div class="leftPaneTitle">与站长微博账号绑定，绑定后将可以回复站长的微博的方式将评论同步到微博</div>
                <div class="bindSNSWrapper" id="unBindWrapper">                	
                    <a class="bindIcon bindSINA" onclick="bindToSNS()"></a>
                    <div class="clear"></div>
                </div>
                <div class="bindSNSWrapper" id="bindedWrapper">                	
                    <a class="bindIcon bindedSINA" onclick="unbindToSNS()"></a>
                    <span class="bindIntro">已绑定</span>
                    <div class="clear"></div>
                </div>        
            </div>-->
        ...
        <div class="rightWrapper">
            ...
            <div class="checkTable">
            	<div class="firstLineTable"><div class="checkNum"></div><div class="checkTitleUp">社交网站</div><div class="checkComment">评论量</div><div class="checkTrace">回访量</div><div class="clear"></div></div>
                <div class="checkItem"><div class="checkNumAll">0</div><a class="checkTitleAll" href="javascript:prepareData(7,'<?php echo $_SESSION['showDomain'];?>');backTitle('sns');">全部社交网站</a><div class="checkComment">
				<?php
				if(isset($snsTrace[0])){
				$commentSNSNum = $snsTrace[0]->n_sina_comments+$snsTrace[0]->n_neteasy_comments+$snsTrace[0]->n_sohu_comments+$snsTrace[0]->n_renren_comments+$snsTrace[0]->n_douban_comments+$snsTrace[0]->n_kaixin_comments+$snsTrace[0]->n_tencent_comments+$snsTrace[0]->n_qq_comments;				
				echo $commentSNSNum;}else{echo 0;}?></div><div class="checkTrace"><?php if($domainTrace[0]->traceamount==NULL){echo 0;}else{echo $domainTrace[0]->traceamount;}?></div><div class="clear"></div></div>                
                <?php 
					$numItem = 1;
				foreach($snsTrace as $row){
                if($row->from_type!=''){;?>
            	<div class="checkItem"><div class="checkNum"><?php echo $numItem++;?></div><a class="checkTitle" href="javascript:changeData(7,'<?php echo $row->from_type;?>','sns');changeTitle('<?php echo $row->from_type;?>','<?php echo $row->from_type;?>','sns');"><?php echo $row->from_type;?></a><div class="checkComment"><?php $str = getTargetComment($row->from_type); echo $row->$str ;?></div><div class="checkTrace"><?php if($row->traceamount==0){echo 0;}else{echo $row->traceamount;}?></div><div class="clear"></div></div>
                <?php }}?>         
            	<div class="clear"></div>
            </div>            
        </div>
...
<?php 
function getTargetComment($from_type){
	switch($from_type){
		case "TENCENT":
		return 'n_tencent_comments';
		break;
		case "QZONE":
		return 'n_qzone_comments';
		break;
		case "SOHU":
		return 'n_sohu_comments';
		break;
		case "RENREN":
		return 'n_renren_comments';
		break;
		case "NETEASY":
		return 'n_neteasy_comments';
		break;
		case "DOUBAN":
		return 'n_douban_comments';
		break;
		case "SINA":
		return 'n_sina_comments';
		break;
		case "KAIXIN":
		return 'n_kaixin_comments';
		break;
		case "email":
		return 'n_email_comments';
		break;
		case "wordpress":
		return 'n_wordpress_comments';
		break;
		default:
		return 'n_wordpress_comments';
		break;
