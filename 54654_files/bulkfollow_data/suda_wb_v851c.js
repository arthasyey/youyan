...
function SUDA(config, ext1, ext2) {
    ...
    SSE = SSL.Core.Event,
    SA = SSL.App;
    var _S_JV_ = "webbug_meta_ref_mod_noiframe_async_fc_:9.12c",
    _S_DPID_ = "-9999-0-0-1";
    ...
    var handler = {
        ...
        _S_gsGID: function() {
            if ("" != _S_GID_) {
                var gid = handler._S_gGID();
                if ("" == gid) {
                	gid = SA._S_gUCk(_S_SID_);
                	handler._S_sGID(gid);
                }
                return gid
            } else {
        ...
        _S_sTEntry: function() {
            ...
            if ("" == SA._S_gUCk(_S_TEntry)) {
                ...
                SA._S_sUCk(_S_TEntry, e, "", "weibo.com")
            }
            ...
            if (_S_PURL_.match(vlogin)) {
                ...
                if ("" != sharehost) {
                    SA._S_sUCk(_S_TEntry, sharehost, "", "weibo.com")
                }
                SA._S_sUCk("appkey", appkey, "", "weibo.com")
            }
        } 
    };
    handler._S_sTEntry();
    ...
    return {
        ...
        _S_gCk: function(ckName) {
            return SA._S_gUCk(ckName);
        },
        _S_sCk: function(ckName, ckValue, ckDays, ckDomain) {
            return SA._S_sUCk(ckName, ckValue, ckDays, ckDomain);
        },
        _S_gGlobalID: function() {
            return handler._S_gGID();
        },
        _S_gSessionID: function() {
            return handler._S_gsSID();
        }
