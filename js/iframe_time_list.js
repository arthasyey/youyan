...
function getPars(){
  var src = document.getElementById("UYScriptTime").src;
  var pos = src.indexOf('?');
...
document.getElementById("uyan_list_time_frame").innerHTML="<div style='width:100px;padding:5px;' id='UYloadingTime'><div style='background:url(http://uyan.cc/images/loadingA.png);width:72px;height:16px;'></div><img sytle='width:80px;height:12px;padding-top:3px;display:block;' src='http://uyan.cc/images/loading.gif'/></div>";
...
function iframeContinue(){
  ...
  var UYSrc = "http://uyan.cc/index.php/youyan_hot_comment?rankType="+encodeURIComponent(UYRankType)+"&title=" + encodeURIComponent(UYPageTitle) + "&url=" + encodeURIComponent(UYPageURL) + "&master_id=" + qstring['UYUserId'] + "&domain=" + encodeURIComponent(UYPageDomain) + "&pageId=" + encodeURIComponent(UYId) + "&pageImg=" + encodeURIComponent(UYImg) + "&pageContent=" + UYPageContent;
	var UYTimeCommentSocket = new easyXDM.Socket({
		...
		container: "uyan_list_time_frame",
		props: {id: "UYTimeWrapperFrame" ,
                scrolling : "no"
				},
		onMessage: function(message, origin){
			var currentHeight = parseInt(message);
			currentHeight +=10;
			document.getElementById("UYTimeWrapperFrame").style.height = currentHeight + "px";
			document.getElementById("UYTimeWrapperFrame").style.width = "100%";
			document.getElementById("UYloadingTime").style.display="none";
		},
		onReady: function() {
			
			UYTimeCommentSocket.postMessage(document.getElementById("uyan_list_time_frame").parentNode.style.width);
		}
	}); 
}
