...
function getPars(){
  var src = document.getElementById("UYScriptHotness").src;
  var pos = src.indexOf('?');
...
var UYImg;
var UYRankType =qstring['rankType'];


document.getElementById("uyan_list_hotness_frame").innerHTML="<div style='width:100px;padding:5px;' id='UYloadingHotness'><div style='background:url(http://uyan.cc/images/loadingA.png);width:72px;height:16px;'></div><img sytle='width:80px;height:12px;padding-top:3px;display:block;' src='http://uyan.cc/images/loading.gif'/></div>";
...
function iframeContinue(){
  ...
  // Setting id, title, content, and img
	...
	var UYSrc = "http://uyan.cc/index.php/youyan_hot_comment?rankType="+encodeURIComponent(UYRankType)+"&title=" + encodeURIComponent(UYPageTitle) + "&url=" + encodeURIComponent(UYPageURL) + "&master_id=" + qstring['UYUserId'] + "&domain=" + encodeURIComponent(UYPageDomain) + "&pageId=" + encodeURIComponent(UYId) + "&pageImg=" + encodeURIComponent(UYImg) + "&pageContent=" + UYPageContent;
	var UYHotCommentSocket = new easyXDM.Socket({
		remote: UYSrc,
		swf: "http://uyan.cc/easyXDM/easyxdm.swf",
		container: "uyan_list_hotness_frame",
		props: {id: "UYHotWrapperFrame",
                scrolling : "no"},
		onMessage: function(message, origin){
			var currentHeight = parseInt(message);
			currentHeight +=10;
			document.getElementById("UYHotWrapperFrame").style.height = currentHeight + "px";
			document.getElementById("UYHotWrapperFrame").style.width = "100%";
			//this.container.getElementsById("UYHotWrapperFrame").style.height = message + "px";
			document.getElementById("UYloadingHotness").style.display="none";	
		},
		onReady: function() {
			UYHotCommentSocket.postMessage(document.getElementById("uyan_list_hotness_frame").parentNode.style.width);
		}
	}); 
}
